...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge { display: inline-block; padding: 0.25rem 0.6rem; font-size: 0.75rem; font-weight: 600; border-radius: 12px; background-color: #17a2b8; color: white; }
        .empty-state { text-align: center; padding: 3rem; color: #6c757d; }
        .empty-state-icon { font-size: 4rem; margin-bottom: 1rem; }
        .filter-section { margin-bottom: 1.5rem; }
        .filter-section select { padding: 0.5rem; border: 1px solid #ced4da; border-radius: 4px; font-size: 0.9rem; }
    </style>
</head>
<body>
<header class="header">
...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            <h2>👨‍🎓 Liste des Élèves (${totalEleves})</h2>
            <a href="${pageContext.request.contextPath}/eleves?action=new" class="btn btn-primary">➕ Nouvel Élève</a>
        </div>

        <c:if test="${not empty filieres}">
            <div class="filter-section">
                <label>Filtrer par filière: </label>
                <select onchange="window.location.href='${pageContext.request.contextPath}/eleves?action=list&filiereId=' + this.value">
        ...
        <c:choose>
            <c:when test="${not empty eleves}">
                <table>
                    ...
                    <c:forEach var="eleve" items="${eleves}">
                        <tr>
                            <td><span class="badge">${eleve.matricule}</span></td>
                            <td><strong>${eleve.prenom} ${eleve.nom}</strong></td>
                            <td>${eleve.email}</td>
                            <td>${eleve.filiere.nom}</td>
                            <td>
                                <div class="actions">
                                    <a href="${pageContext.request.contextPath}/eleves?action=view&id=${eleve.id}" class="btn btn-primary btn-sm">👁️ Voir</a>
                                    <a href="${pageContext.request.contextPath}/eleves?action=edit&id=${eleve.id}" class="btn btn-success btn-sm">✏️ Modifier</a>
                                    <a href="${pageContext.request.contextPath}/eleves?action=delete&id=${eleve.id}" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet élève ?')">🗑️ Supprimer</a>
                                </div>
                            </td>
                        </tr>
                    </c:forEach>
            ...
            <c:otherwise>
                <div class="empty-state">
                    ...
                    <a href="${pageContext.request.contextPath}/eleves?action=new" class="btn btn-primary">➕ Créer un Élève</a>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
