...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            <h2>📖 Liste des Cours (${totalCours})</h2>
            <a href="${pageContext.request.contextPath}/cours?action=new" class="btn btn-primary">➕ Nouveau Cours</a>
        </div>

        <c:choose>
            <c:when test="${not empty coursList}">
                <table>
                    <thead>
                    <tr>
                        <th>Code</th>
                    ...
                    <c:forEach var="cours" items="${coursList}">
                        <tr>
                            ...
                            <td><strong>${cours.intitule}</strong></td>
                            <td>${cours.credits != null ? cours.credits : '-'}</td>
                            <td>${cours.description != null ? cours.description : '-'}</td>
                            <td>
                                <div class="actions">
                                    <a href="${pageContext.request.contextPath}/cours?action=edit&id=${cours.id}" class="btn btn-success btn-sm">✏️ Modifier</a>
                                    <a href="${pageContext.request.contextPath}/cours?action=delete&id=${cours.id}" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce cours ?')">🗑️ Supprimer</a>
                                </div>
                            </td>
                        </tr>
                    </c:forEach>
            ...
            <c:otherwise>
                <div class="empty-state">
                    ...
                    <a href="${pageContext.request.contextPath}/cours?action=new" class="btn btn-primary">➕ Créer un Cours</a>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
